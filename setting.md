...
- **Large — axial ring-stack.** Multiple independent rings mounted along a shared, despun axle like wheels on an axle, always added in matched clockwise/counter-clockwise pairs so net angular momentum on the axle stays at zero without needing a flywheel. Only Nova Reach's wing (≈25,000–35,000), hosting the bulk of the network's population and all three branch mouths, has ever grown into this tier — see the dedicated subsection below.
Because Sat-48 has operated for 65+ years, wings that have grown into a higher tier can plausibly still show physical evidence of the earlier stage — an old, disused arm-pod section standing near Nova Reach's current ring-stack, for instance — while Enlil's wing has simply never left tier one.
 
### Ring geometry and axial growth (Nova Reach wing)
 
Nova Reach's habitat anchor is the one wing that outgrew a single ring, and its structure reads as a visible record of the colony's own growth rather than a single design executed all at once.
 
- **Target gravity and rotation rate:** every ring, regardless of size or era, is built to deliver ~0.5g at the rim at a rotation rate around 2 rpm — slow enough to stay within comfortable Coriolis limits for lifelong residents, transferring crew, and cargo alike. Since a = ω²r, holding both constants fixed means holding radius roughly fixed too (≈120 m), so every ring across every era of construction feels the same to someone walking from one to another.
- **Axial stacking, not radial widening:** rather than one wide double-ring structure, individual rings mount at intervals along a shared despun spine. Each ring is its own separately built, separately bearing-mounted structure. Momentum cancellation is handled purely by pairing — every new ring goes up alongside a twin spun in the opposite direction — so once a wing has grown past its first ring, it never needs a non-habited counterweight again; the counterweight tier (medium) is a stopgap for wings that have only ever had one ring.
- **Exponential growth, changing ring geometry:** the earliest ring pairs, built when the wing's population was still in the hundreds to low thousands, are small, thin tori — modest cross-section, just enough to prove the structure and house a small crew cheaply. As the population and Trelium industry (§10.4) scaled over Sat-48's 65+ year history, each subsequent ring pair has been built larger than the last, and the largest, newest pairs trade the thin-torus shape for a long hollow cylinder: same ~120 m radius (so the gravity and rotation rate never change), but greatly extended along the axial dimension. A torus's usable floor area is capped by its cross-sectional loop; a cylinder of the same radius can simply be built longer along the axle to add floor area, without touching the gravity profile at all — the more space-efficient shape once a ring pair is large enough to justify the added structural mass.
...
**Gravity target and rotation rate.** Using the same real-physics reasoning as the eventual network standard (a = ω²r) but rounding to a clean 2.000 rpm rather than the ~1.93 rpm the later full-ring standard converges on, 0.5g at the rim requires a major radius of **111.8 m** — close to, but numerically distinct from, the 120 m figure locked in for all subsequent ring construction above. This is treated as a real historical detail: Year 0's engineers worked from a rounder, less-refined design pass; once Sat-48 began mass-producing full rings at scale, radius and rpm were tightened to the precise 120 m/1.93 rpm figures that hold for every ring built afterward. The two numbers are not meant to be reconciled into one — they represent two different engineering moments 65+ years apart.
