...
- Confirm whether §10.6's "martyred charismatic leader" reference and Deschamps (§10.8) are formally the same figure, or whether the martyr figure is distinct.
- Decide whether Nova Reach / this cluster has military or strategic value pre-collapse, or remains purely economic/civilian.
- Ereshkigal's transportation architecture is locked (§13.4), but exact core dry mass, crew capacity, and payload figures for the PDV-2 are still illustrative placeholders, not locked canon — as is the import-side capture system symmetric to the MD-C export cycle.
