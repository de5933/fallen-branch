...
**Pressure hull: cabin pressure, hull thickness, MMOD shielding.** Cabin pressure is locked at **101.3 kPa (1 atm), 21%/79% O₂/N₂** — a full sea-level shirtsleeve mix, chosen over a lower-pressure O₂-enriched atmosphere because docking is always done despun and stationary (no EVA-prebreathe savings to be had), leaving comfort over a 70–90 day transit as the deciding factor.

Hull thickness is derived from thin-wall pressure vessel theory, σ_hoop = ΔP·r/t, at r = 2 m (4 m diameter): with 2219-T87 aluminum (yield ≈ 352 MPa, the Shuttle External Tank alloy — no exotic material, locally sourceable per [§2.1](#2.1)) and a standard NASA metallic-pressure-vessel margin of FS 4 on yield, membrane-only thickness comes to ~2.3 mm. Rounded up for weld margin and buckling stiffness, **skin is locked at 4 mm, uniform gauge, with internal frame/stringer ribs** rather than a bare membrane — shell mass works out to roughly 4–8 t including framing, a plausible ~14–15% of the pod's 53.45 t budget.

The pod's two end caps are shallow **hemispherical domes**, not flat bulkheads — a sphere carries half the hoop stress of a cylinder at the same r and ΔP, making the dome the structurally efficient shape at the ends regardless of shielding. Caps are held to the same 4 mm gauge as the cylinder for tooling commonality rather than optimized thinner.

**Whipple shields, fore and aft.** The mid-course flip ([§1.4](#1.4)) means the pod's leading (ram) face swaps mid-transit — fore end leads during the first-half burn, aft end leads after the flip — so both domes, not just one, need standoff debris shielding. Each is a classic single-bumper Whipple shield: a sacrificial 1.5 mm aluminum bumper layer standing off 12 cm from the primary hull. An impactor shatters or vaporizes against the bumper, spreading the resulting debris cloud's energy over a much larger area before it can reach the pressure hull. Total shield mass across both domes (≈50 m² of bumper plus standoff struts) comes to roughly 275 kg — negligible against the pod's mass budget.

At the aft dome, the airlock sits behind a **hinged/sliding segment of the bumper shield** rather than a fixed cutout — closed through the whole cruise like the rest of the dome, and swung open only for the two actual docking events (departure, arrival), since the mid-course flip itself involves no docking. Minimal moving parts, consistent with Farfield's preference for fewer moving parts already established in the galley table design.

**Interior partition walls.** The pod is one continuous pressurized volume — no internal pressure bulkheads — so interior walls carry only their own mounting and acoustic loads, not hoop stress. This is a deliberate simplicity-first choice: compartmentalized damage-control bulkheads would add real structural mass against a threat window (mid-transit MMOD) already mitigated by the fore/aft Whipple domes, and side-wall debris strikes are low-probability relative to end-on impacts for a ship traveling axially. Interior partitions are **aluminum honeycomb sandwich panel, ~3 cm thick** — thin aluminum face sheets bonded to an aluminum honeycomb core, the same composite paneling already described for the corridor walls, chosen for stiffness-to-mass and acoustic damping rather than pressure integrity (consistent with cabin partitions already established as thin enough to bleed sound through). Total partition mass across the pod comes to roughly 600 kg — trivial against the 53.45 t budget.
...
- **Ascent — two-mode propulsion, one airframe.** Low-altitude climb runs on electric ducted-fan propulsion, staying in the same family as the network's other electric-climb vehicles ([§1.2](#1.2)) — and favorable here: Olokun's dense lower atmosphere (surface density ~4.7 kg/m³ at the locked 155 K/3 bar, [Cosmology §5.2](Cosmology.md#5.2)) gives roughly **~4.8× Earth's payload per unit wing area** at matched airspeed, recalculated from the pre-155K-revision figure of 9× — still a genuine advantage, just smaller now that the warmer, less dense atmosphere is locked in. The vehicle stages functionally (not physically — same airframe, switched propulsion mode) to a methalox rocket once airspeed and altitude exceed what the fan can efficiently push against, continuing the climb to orbit. *The ~14.1 km staging-altitude figure predates the temperature/scale-height revision (scale height moved from 4.71 km to ~8.11 km) and needs rechecking — not yet re-derived; see [Open Questions](Open_Questions.md).*
- **Propellant.** Methane is locally sourced — the same hydrocarbon extraction already supporting Olokun's economy ([Infrastructure §2](Infrastructure.md#2)) — and sits naturally near liquid storage temperature at Olokun's ambient 155 K, since that's the same equilibrium temperature its source seas sit at, removing any need for dedicated methane liquefaction infrastructure. LOX has no local hydrocarbon-side source; electrolysis of Olokun's local permafrost ice ([Infrastructure §2](Infrastructure.md#2), Olokun subsection) is the practical source, a standing dependency distinct from the fuel side.
- **Reentry.** Olokun's dense atmosphere gives **~3.65× Earth's column mass** (P/g = 38,217 kg/m² vs. Earth's ~10,480 kg/m², at the locked 155 K/3 bar) — recalculated from the earlier 2.5–6× range now that pressure is a fixed point rather than a range — providing strong aerobraking on descent, shedding a large fraction of orbital velocity before the vehicle needs to rely on structure-taxing active braking.
