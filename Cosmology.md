...
~9–10 AU, axial tilt ~22° (moderate, Earth-like), orbital eccentricity ~0.15–0.20 (a second, overlapping source of seasonal variation atop the tilt), orbital period ~27–30 years. Surface gravity 0.8g (7.85 m/s²) is the anchor figure, implying a mass of ~0.757 M⊕ at a 6,200 km radius and a bulk density of ~4,528 kg/m³ — less dense than Earth's average, consistent with a rocky world with a smaller metallic core fraction and a water-ice-rich crust rather than Earth's substantial metallic core.

**Surface temperature ~155 K — locked, and warmer than a naive equilibrium estimate would give.** At Olokun's weak insolation (~7–9 W/m²), a thin methane/ethane atmosphere alone wouldn't sustain this; the mechanism is collision-induced absorption — at real density, gas collisions briefly activate otherwise IR-inactive vibrational modes, so greenhouse trapping scales faster than linearly with pressure. This produces a self-reinforcing loop (denser atmosphere → more trapping → warmer surface → higher saturation vapor pressure → still denser atmosphere) that settles at a stable point well above the naive estimate, rather than running away Venus-style — there isn't enough incoming energy for that. **155 K is still lethal to an unprotected human in well under a minute; the temperature increase changes the planet's internal stability, not its danger to people.**

**Atmosphere: locked at 70:30 methane:ethane by mole (vapor phase), ~3 bar total pressure, mean molar mass 20.25 g/mol, scale height ~8.11 km** (recalculated at 155 K; the atmosphere is pure methane/ethane with no third diluent gas — nitrogen or another non-condensable carrier was considered and rejected in favor of the temperature-driven solution, which preserves the locked composition ratio). At 155 K and 3 bar, this composition is a genuine, physically consistent liquid/vapor equilibrium — unlike a 90 K/2–5 bar reading of the same numbers, which a pure two-component methane/ethane system can't actually sustain (its saturation ceiling near 90 K is only ~0.03–0.13 bar).

**Ocean: ~26% surface coverage, scattered seas rather than a world-ocean — Olokun is mostly land.** This reframes the "ocean world" framing considerably: what liquid exists is genuinely uncommon, not the dominant terrain feature the name might suggest. Where seas do occur, the liquid itself works out to roughly **20% methane / 80% ethane by mole** — ethane-dominant, since its much lower volatility at 155 K requires it to dominate the liquid for the locked 70:30 vapor ratio to hold above it. Methane and ethane are fully miscible, so this ratio holds uniformly through a sea's depth — no stratified layering by hydrocarbon type, though solid particulate (settled haze/organics or ice sediment) may still collect on a seabed. Colonists call this liquid **"oil"** day to day, reaching for **"tar"** specifically when its high viscosity is the point being made.

### Sky and Landscape

**Sky:** still hazy and diffuse — the atmosphere's methane photochemically hazes under whatever weak UV/cosmic-ray flux reaches this far out, producing an opaque orange-brown dome with no hard shadows anywhere and no true disc to the star, just a dim, directionless smudge. Obatala's star, at ~9–10 AU, is already faint (comparable to Saturn's remove from Sol, ~7–9 W/m²) before the haze further dims it.

**Land dominates — a cold, rust-colored desert, not an ice-and-ocean world.** With seas covering only ~26% of the surface, most terrain is wind-worn permafrost: rock and water-ice bedrock scoured by the dense 3-bar atmosphere into smooth, rounded formations rather than sharp alpine relief — closer to a desert's wind-sculpted mesas than jagged glacial terrain. No vegetation; muted greys and rust-browns under the haze.

**The seas, where they occur, are dark, thick, and slow-moving** — colonists' "oil," running noticeably more viscous than water, rolling in sluggish, low waves rather than breaking sharply. Genuinely striking against the flat, dry land around them: the one place anything visibly moves.

**Settlement is buried and quiet from outside.** Structures barely break the skyline — low, rounded access domes, comms masts, and anchored extraction platforms out on the nearest sea are essentially all that's visible; everything load-bearing sits underground in the permafrost. The hydroponic domes register as a faint warm glow at distance, easy to miss against the haze rather than a dramatic beacon.

Seasons run in multi-year swings (27–30-year orbit, moderate tilt plus real eccentricity), so the mood of the landscape shifts on a generational rather than annual clock — most Olokunites live through less than one full seasonal cycle in a working lifetime.

### View From Orbit

Essentially featureless from orbit: a smooth, hazy orange-tan globe with the dense methane/ethane atmosphere hiding every surface detail. At most, the faintest tonal variation hints at scattered sea versus permafrost land beneath the haze — with seas covering only ~26% of the surface, the visible pattern (were the haze thin enough to show it) would read as isolated dark patches on a much larger pale-rust landmass, not a world-ocean. No extraction platforms visible — the haze swallows everything.
...
Airless and tidally locked directly around the white dwarf. At ~0.0125 AU with Enlil A's actual radius, the white dwarf's angular diameter works out to roughly 0.4° — close to our own Sun's apparent size from Earth (0.53°) — but at Teff 15,140K, blue-white and far more intense, not a gentle amber disc. On the dayside that's a genuinely dangerous, eye-searing presence in an otherwise jet-black vacuum sky with unblinking stars — harsh enough that even the carbonatite deposit is worked from the nightside rather than the dayside, despite the added convenience dayside access would otherwise offer. Enlil B is visible as an ordinary point far off; the debris ring may show as a faint band if aligned. The settlement, reactor, and extraction workings all sit deep-nightside, well clear of the terminator ([Infrastructure §3](Infrastructure.md#3)/[Technology §1.3](Technology.md#1.3)) — nobody lives or works routinely under open sky. The one place the white dwarf remains a visible, alien presence even from the nightside is as a small, ferociously blue-white point low in the sky, eye-searing if stared at directly. Topside exposure on the nightside itself is limited to EVA maintenance work. Landscape: blasted, phosphorus-rich rock, nightside extraction workings and worked-tailings piles, and buried/bermed habitat structures rather than anything resembling Lugh's open domes.
