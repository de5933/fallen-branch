...
*Split from the master worldbuilding bible (`setting.md`) by topic. Section markers like [§7.1](#7.1) refer to the original numbering and may now point to content living in a different file — cross-check against the other topic files if a reference doesn't resolve locally.*
...
- Engineers "inflate" a seed by pumping fermions and electric flux through it, growing the throat. Inflation is staged by cost and risk (see [§3](#3)).
...
- Communication between gate ends is effectively near-instant via fiber-optic threading of the wormhole; only in-system distances incur light-speed delay (see [§14](Infrastructure.md#14) for in-system comm delay figures).
...
- **Stage 3 — Full expansion.** Reserved for confirmed winners. The expensive, committed step: inflating the throat to the network's 2.5 m passenger/cargo standard ([§2.1](Infrastructure.md#2.1)) — sized for the counterweighted twin-train transit system, never for a ship's hull. The true point of no return, where the corporation is on the hook for full colony logistics.
...
Because the throat has essentially no mechanical cross-section ([§2](#2)), engineers can't aim matter into it the way you'd load a target. Instead, a precisely phase-matched fermion stream is fired at the seed, and the real question is whether it couples coherently into the existing entangled state (successful inflation) or simply scatters off as waste heat (failed coupling) — closer to stimulated emission in a laser than to marksmanship. This is exactly what the threading-field response wormhole engineers test for ([§10.3](Culture.md#10.3)): whether a given seed's state is currently receptive to coherent addition.
...
**Near mouth — the one Farfield chooses where to harvest: ~100% usable.** Real planetary-science thresholds put differentiation onset around 100–200 km body radius, while ring particles and rubble-pile moonlets top out in the tens-to-hundreds-of-meters range — three to four orders of magnitude too small, ever, to have melted and sorted a core. A seed harvested from ring or small-asteroid material ([§10.4](Economy.md#10.4)) is essentially guaranteed to still be sitting exactly where it landed, fully accessible, never buried by differentiation. This is why ring/asteroid siting ([§4.1](Infrastructure.md#4.1)) is doubly justified: it's the only class of host that guarantees a usable near mouth *and* it happens to supply free ballast ([§2.1](Infrastructure.md#2.1)).
...
All three systems are reached via Sat-48 in Nova Reach and were established before the eventual wormhole collapse, discovered and founded via the same staged-inflation method ([§3](#3)) using seeds harvested locally from Goibniu's own ring band. Each uses a distinct mythological naming convention, chosen deliberately to avoid the overused Greco-Roman well: Irish (Danu), Yoruba (Obatala), and Mesopotamian (Enlil). Each system's own Sat-48-designated wing — Safford — is sited on a distinct local body; see each subsection below and [§13](Travel.md#13)–14 for the resulting transit and communication figures, and [§4.1](Infrastructure.md#4.1) for each wing's habitat tier.
...
- Obatala f — gas giant, ~8 AU. A minor Trelium source at best, nothing like Goibniu's rich helium enrichment ([§6](#6)) — not presently worth the infrastructure to exploit.
...
*Ereshkigal's status as the network's key phosphorus/starvation-leverage chokepoint (see §9.5 — **broken reference, no §9.5 exists**; likely candidates are [Economy.md [§17.1](Economy.md#17.1)](./Economy.md#17.1) or [Politics.md [§10.5](Politics.md#10.5)](./Politics.md#10.5), both of which discuss this chokepoint) is reinforced by its harsh, small-population character — a resource stranglehold held by very few people.*
<a name="9"></a>
# 9. The Severance Event
 
The triggering cause originates specifically on the Sol side (Sat-39 Sol). Cause unknown and unknowable in-universe (the failure occurs on the far side of the gate from Nova Reach's perspective, or the initiating event is otherwise never determined). Because both segments of Sat-39 share one mass ([§9.1](#9.1), [§9.3](History.md#9.3)), both convert to black holes at once, each with a mass equal to the mass of the gate mouth it replaced, per local conservation of mass.
...
- **The high edge — mass climbing toward charge.** Reached either by a single-transit overshoot (an unbalanced crossing's mass approaching the mouth's own, [§2.1](Infrastructure.md#2.1)) or by chronic net-import drift (a mouth on the receiving end of a long-term uncorrected trade imbalance, [§2](#2)).
